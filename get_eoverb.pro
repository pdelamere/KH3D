...
cwpi = cwpi
rhoi = sqrt(beta)*cwpi
vA = b0_top/sqrt(!pi*4e-7*mproton*np_top/1e9)/1e3  ;km/s
...
;   b1 = b1*mproton/q     ;T
   ...
   Ef1(*,*,*,0) = Ef(*,*,*,0) - up(*,*,*,2)*b0_top*q/mproton
   Ef1(*,*,*,2) = Ef(*,*,*,2) - up(*,*,*,0)*b0_top*q/mproton
   Ef1(*,*,*,1) = Ef(*,*,*,1)

;   print,'Ef...',max(Ef),max(up(*,*,*,0)*b0_top*1.6e-19/1.67e-27)
;   stop
   x0 = nxz/2 + 1
   x1 = x0-nxz/2
   x2 = x0+nxz/2-1
   ...
   kx = kx(0:N21x-1)
   kxarr = kx
   eoverb_tot = fltarr(n_elements(kx))
   eoverb_stdev = eoverb_tot
   eoverb_tot_cnt = 0.0
   
   for jj = 1, ny-2,1 do begin
;   jj = ny/2
      x0 = nxz/2 + 1
      while((x0 + nxz/2) lt nx-2) do begin
         ...
         fx = fft(reform(b1(x1:x2,jj,z1:z2,0)))
         Efx = fft(reform(Ef1(x1:x2,jj,z1:z2,0)))
         
         fz = fft(reform(b1(x1:x2,jj,z1:z2,2)))
         Efz = fft(reform(Ef1(x1:x2,jj,z1:z2,2)))
         ...
         for i = 0,n_elements(kx)-1 do begin
            for k = 0,n_elements(kz)-1 do begin
               ...
               eoverb(i,k) = abs(Efz(i,k)^(2.))/abs(fx(i,k)^(2.))
...
;               (k_perp*rhoi)*(psd(i,k)*k_perp)/sqrt(muo^3*rho)
         ...
         eoverb_sum = fltarr(n_elements(kx))
         eoverb_cnt = fltarr(n_elements(kx))
         eoverb_ave = fltarr(n_elements(kx))
         dk = kx(1)-kx(0)
         for i = 0,n_elements(kx)-1 do begin
            for k = 0,n_elements(kz)-1 do begin
               k_perp = sqrt(kx(i)^2 + kz(k)^2)
               ...
               eoverb_sum(wh(0)) = eoverb_sum(wh(0)) + eoverb(i,k)
               eoverb_cnt(wh(0)) = eoverb_cnt(wh(0)) + 1.0
            endfor
         endfor
...
;      plot_q,kx,beta,cwpi,pwr_arr_sum,pwr_kaw_arr_sum
      ...
      eoverb_ave = eoverb_sum/eoverb_cnt
      plot,kx*s.rhoi,sqrt(eoverb_ave),/ylog,/xlog,xrange=[0.1,20]
      oplot,kx*s.rhoi,vA*sqrt(1+(kx*s.rhoi)^2),linestyle =1
      ;wait,0.1
      eoverb_tot = [eoverb_tot,eoverb_ave]
      kxarr = [kxarr,kx]
      eoverb_tot_cnt = eoverb_tot_cnt + 1.0
   endfor

   print,eoverb_tot_cnt
   eoverb_geomean = fltarr(n_elements(kx))
   for i = 0,n_elements(kx)-1 do begin
      wh = where(kxarr*s.rhoi eq kx(i)*s.rhoi)
      eoverb_geomean(i) = geo_mean(sqrt(eoverb_tot(wh(1:*))))
      eoverb_stdev(i) = stdev(sqrt(eoverb_tot(wh(1:*))))/sqrt(eoverb_tot_cnt)
;      print,'eoverb....',sqrt(eoverb_tot(wh)>0)
;      stop
   endfor
   ;p = plot(kxarr*s.rhoi,sqrt(eoverb_tot),'r.',/ylog,/xlog,xrange=[1,20],yrange=[10,4000])
   p1 = errorplot(kx*s.rhoi,eoverb_geomean,eoverb_stdev,'2k',/ylog,/xlog,xrange=[1,16],yrange=[200,3000],name='Simulation')
   p1.xtitle='$k_\perp \rho_i$'
   p1.ytitle='$E_z/B_x$'
   p2 = plot(kx*s.rhoi,vA*sqrt(1+(kx*s.rhoi)^2),'2r',/overplot,name='Theory')
